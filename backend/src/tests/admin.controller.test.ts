...
const TEST_MONGODB_URI = 'mongodb://localhost:27017/comp5347_admin_test';
...
async function createUser(role: 'user' | 'admin' = 'admin', suffix: string | number = Date.now()): Promise<UserDocument> {
  const user = new User({
...
function validQuestionPayload(overrides: Partial<QuestionSeed> = {}): QuestionSeed {
  return {
