...
/** Mongo unique-index violation surfaced by Mongoose. */
...
const buildQuestionTextRegex = (questionText: string) => {
...
const getTextQualityError = (
  label: string,
  value: string,
  { min = 1, max, allowEmpty = false }: TextQualityOptions = {}
): string | null => {
...
/** Untrusted request-body shape: every field is checked at runtime before use. */
type UnknownQuestionInput = Record<string, unknown>;

/** Payload shape guaranteed once isValidQuestionPayload has returned null. */
interface ValidQuestionInput {
  questionText: string;
  options: string[];
...
const isValidQuestionPayload = (question: UnknownQuestionInput | null | undefined): string | null => {
  ...
  if (question.explanation !== undefined) {
    ...
    if (question.explanation.trim().length > EXPLANATION_MAX_LENGTH) {
...
const findDuplicateQuestionByText = async (questionText: string, excludeId?: string) => {
  const query: mongoose.FilterQuery<IQuestion> = {
...
const getBulkDuplicateErrors = async (questions: NormalizedQuestion[]): Promise<BulkValidationError[]> => {
  const validationErrors: BulkValidationError[] = [];
  const firstQuestionIndexByText = new Map<string, number>();
 ...
 * Update one question.
 */
const updateQuestion: RequestHandler<{ id: string }> = async (req, res, next) => {
  try {
 ...
 * Delete one question.
 */
const deleteQuestion: RequestHandler<{ id: string }> = async (req, res, next) => {
  try {
    if (!isValidObjectId(req.params.id)) {
      return res.status(400).json(fail('Invalid question id'));
    }
 ...
 */
const toggleQuestionStatus: RequestHandler<{ id: string }> = async (req, res, next) => {
  try {
    if (!isValidObjectId(req.params.id)) {
      return res.status(400).json(fail('Invalid question id'));
    }
 ...
 */
const bulkImportQuestions: RequestHandler = async (req, res, next) => {
  try {
