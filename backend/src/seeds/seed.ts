...
function validateWizardQuestion(row: unknown, index: number): asserts row is WizardQuestionRow {
  ...
  if (!('questionText' in row) || !row.questionText || typeof row.questionText !== 'string') {
    throw new Error(`${label} must include questionText`);
  }
  if (!('options' in row) || !Array.isArray(row.options) || row.options.length !== 4) {
...
async function seedUser({ username, email, role, password }: DemoUser): Promise<void> {
