...
4. **Connect** → *Drivers* → copy the connection string. It looks like:
   `mongodb+srv://<user>:<password>@cluster0.xxxx.mongodb.net/survival_sydney?retryWrites=true&w=majority`
   Make sure the database name (`survival_sydney`) is in the path.

## 2. Services — Render Blueprint

1. Push this repo (with `render.yaml`) to GitHub.
2. In Render → **New** → **Blueprint** → connect the repo. Render reads `render.yaml` and proposes `survival-sydney-api` and `survival-sydney-web`.
...
|---|---|---|
| `survival-sydney-api` | `MONGODB_URI` | the Atlas string from step 1 |
| `survival-sydney-api` | `CLIENT_ORIGIN` | the web URL, e.g. `https://survival-sydney-web.onrender.com` |
| `survival-sydney-web` | `VITE_API_BASE_URL` | the API URL + `/api`, e.g. `https://survival-sydney-api.onrender.com/api` |
...
The API has no questions until seeded. Easiest option: open a **Shell** on the `survival-sydney-api` service in the Render dashboard and run:

```bash
node dist/seeds/seed.js
```

Seeds the 57-question Survival Sydney bank plus demo accounts `admin / AdminPass123` and `player1 / PlayerPass123`.

## 5. Verify

- `https://survival-sydney-api.onrender.com/api/health` → `{"success":true,"data":{"status":"ok"}}`
