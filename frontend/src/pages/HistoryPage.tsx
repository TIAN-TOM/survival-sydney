...
import { formatReviewCategory } from '../components/quiz/reviewFormatUtils.ts';
import { useQuiz } from '../contexts/QuizContext.tsx';

const CHIP_ORDER = ['renting', 'transport', 'safety', 'scam', 'food', 'general'];

function chipToneClass(topic: string) {
...
function pickBestAttempt(rows: HistoryRow[]): HistoryRow | null {
...
function HistoryAttemptCard({ attempt, attemptNo, navigate, staggerIndex = 0 }: HistoryAttemptCardProps) {
...
function HistoryPage() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const data = await api.get<HistoryRow[]>('/quiz/history');
  ...
  const stats = useMemo(() => {
    ...
    const bestTotal = best && best.totalQuestions > 0 ? best.totalQuestions : 10;
