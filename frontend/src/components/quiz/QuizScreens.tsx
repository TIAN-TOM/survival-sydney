...
import { BRAND, resultRankBand } from '../../config/brand.ts';
import ThemeFloatingToggle from '../ThemeFloatingToggle.tsx';
import { quizStartHeroImageSrc } from '../../quizBrandAssets.ts';
import QuizFramedPanel from './QuizFramedPanel.tsx';
import { formatReviewCategory } from './reviewFormatUtils.ts';
import GameplayHudPortal from '../GameplayHudPortal.tsx';
...
const TOPIC_MAP: Record<string, { cls: string; label: string }> = {
...
function stopsForCount(n: number) {
...
function TopicPill({ topic, style }: { topic: string; style?: CSSProperties }) {
...
function JourneyStrip({ total, current }: { total: number; current: number }) {
...
function CandleIcon({ state }: { state: 'done' | 'current' | 'locked' }) {
  return (
...
function QuizProgressStrip({ total, current }: { total: number; current: number }) {
  const stops = useMemo(() => stopsForCount(total), [total]);
  const denom = Math.max(total - 1, 1);
...
export function QuizGateScreen({ authChecking = false }: { authChecking?: boolean }) {
...
export function ResultScreen() {
  ...
  // The final 0-threshold entry always matches, so find() cannot return undefined here.
