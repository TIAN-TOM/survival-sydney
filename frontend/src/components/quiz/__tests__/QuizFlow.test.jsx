...
vi.mock('../../../contexts/QuizContext.jsx', () => ({
  useQuiz: () => quizState.value,
}));

vi.mock('../QuizScreens.jsx', () => ({
  CalculatingScreen: () => <div>calculating</div>,
  QuizGateScreen: ({ authChecking }) => <div>{authChecking ? 'checking auth' : 'guest gate'}</div>,
  QuizScreen: () => <div>quiz</div>,
  ResultScreen: () => <div>result</div>,
  StartScreen: () => <div>start quiz</div>,
}));

vi.mock('../QuizWorldBackground.jsx', () => ({
