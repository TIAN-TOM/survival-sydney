...
import type { BulkImportResult, QuestionPayload } from '../types.ts';
...
function parseImportPayload(jsonText: string): QuestionPayload[] {
  let parsed: unknown;
  ...
  // The cast only narrows for the property read; a non-object payload still throws the
  // same runtime TypeError it always did before reaching the Array.isArray check.
  const questions: unknown = Array.isArray(parsed) ? parsed : (parsed as { questions?: unknown }).questions;
  ...
  const validatedQuestionEntries: { index: number; question: z.infer<typeof questionImportSchema> }[] = [];
...
export default function BulkImport({
  ...
  onImportSuccess?: (result: BulkImportResult) => void | Promise<void>;
}) {
  ...
  const submitImport = async (values: z.infer<typeof bulkImportFormSchema>) => {
    try {
      setImporting(true);
      setMessage(null);

      const questions = parseImportPayload(values.jsonText);
      const result = await api.post<BulkImportResult>('/admin/questions/bulk-import', { questions });
