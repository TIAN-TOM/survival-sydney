...
export default function Register() {
  ...
  const onSubmit = async ({ username, email, password }: z.infer<typeof registerSchema>) => {
    setServerError('');
    try {
      ...
      if (status === 409) {
        setServerError(message || 'That username or email is already registered.');
        return;
      }
      if (status === 400) {
        setServerError(message || 'Please check the form and try again.');
        return;
      }
